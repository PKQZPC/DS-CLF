...
class MultiAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        K = K.view(batch_size, seq_len, self.num_heads, self.d_head).transpose(1, 2)  # [batch_size, num_heads, seq_len, d_head]
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=100):  
        ...
        pe = self._get_positional_encoding(max_len, d_model)
    ...
    def forward(self, x):
        ...
        pe = self.pe[:seq_len0, :seq_len1, :seq_len2]
...
class Classifier_CL(nn.Module):
    def __init__(self, num_layers, num_class=Num_class, d_model=128, num_heads=8):
        super(Classifier_CL, self).__init__()
        self.embedding = nn.Embedding(20, 128)
        self.position_embedding = PositionalEncoding(d_model)
        self.MutiA = MultiAttention(d_model, num_heads)
